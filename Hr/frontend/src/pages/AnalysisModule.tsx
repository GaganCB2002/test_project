...
export default function AnalysisModule() {
  ...
  if (loading) return <div className="p-8 text-slate-500">Loading Tech Lead Analysis...</div>;
    ...
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-slate-500 mt-1">Real-time performance signals from the Tech Lead Analysis Engine.</p>
        </div>
      </div>
      ...
      <div className="grid lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 bg-white rounded-[32px] border border-slate-200 p-8 shadow-sm">
          ...
          <div className="prose prose-slate max-w-none">
            <pre className="bg-slate-50 p-6 rounded-2xl border border-slate-100 text-sm font-mono text-slate-600 overflow-x-auto h-[500px]">
        ...
        <div className="space-y-6">
          <div className="bg-indigo-600 rounded-[32px] p-8 text-white shadow-lg shadow-indigo-200">
          ...
          <div className="bg-white rounded-[32px] border border-slate-200 p-8 shadow-sm">
            ...
            <div className="space-y-4">
              {records.filter(r => r.quality === 'High').slice(0, 4).map(record => (
                <div key={record.id} className="flex items-start gap-3 p-3 rounded-2xl hover:bg-slate-50 transition">
                  <div className="h-10 w-10 rounded-xl bg-emerald-50 flex items-center justify-center text-emerald-600">
                    <Code2 className="h-5 w-5" />
                  </div>
                  <div>
                    <p className="text-sm font-bold text-slate-900">{record.title}</p>
                    <p className="text-xs text-slate-500">{record.developer} • {record.timeSpent}</p>
                  </div>
                </div>
              ))}
