        ...
        <div className="grid gap-4 lg:grid-cols-[0.7fr_1.3fr]">
          <div className="space-y-3">
            {platform.employees.departments.map((department) => (
              <div key={department.department} className="rounded-2xl border border-slate-200 bg-white px-4 py-4">
                <p className="font-semibold text-slate-900">{department.department}</p>
                <p className="mt-1 text-sm text-slate-500">{department.count} employees</p>
              </div>
            ))}
          </div>
      ...
      <SectionCard title="Employee directory" subtitle="Personal and job details across the active workforce.">
        <div className="grid gap-4 lg:grid-cols-2 xl:grid-cols-3">
          {platform.employees.employees.map((employee) => (
            <article key={employee.id} className="rounded-[24px] border border-slate-200 bg-white p-5">
              ...
              <div className="mt-4 flex flex-wrap gap-2">
                {employee.skills.map((skill, idx) => (
                  <span key={`${skill}-${idx}`} className="rounded-full bg-slate-100 px-3 py-1 text-xs font-medium text-slate-600">
                    {skill}
                  </span>
                ))}
              </div>
            ...
            <tbody>
              {platform.attendance.leaveRequests.map((request) => (
                <tr key={request.id} className="border-t border-slate-100">
                  <td className="px-4 py-4 font-semibold text-slate-900">{request.employeeName}</td>
      ...
      <section className="grid gap-4 md:grid-cols-2 xl:grid-cols-[0.85fr_1.15fr]">
        ...
        <SectionCard title="Review outcomes" subtitle={`Average review score ${platform.performance.averageReview}/5`}>
          <div className="grid gap-4">
            {platform.performance.records.map((record) => (
              <article key={record.employeeId} className="rounded-[24px] border border-slate-200 bg-white p-5">
                <div className="flex items-start justify-between gap-4">
                  <div>
                    <p className="font-semibold text-slate-900">{record.employeeName}</p>
                    <p className="text-sm text-slate-500">{record.coachNotes}</p>
                  </div>
                  <StatusBadge label={record.promotionReadiness} />
                </div>
                <div className="mt-4 grid gap-3 sm:grid-cols-3">
                  <div className="rounded-2xl bg-slate-50 px-4 py-3 text-sm">Goals {record.goalCompletion}%</div>
                  <div className="rounded-2xl bg-slate-50 px-4 py-3 text-sm">KPI {record.kpiScore}</div>
                  <div className="rounded-2xl bg-slate-50 px-4 py-3 text-sm">360 {record.feedback360}</div>
                </div>
                <div className="mt-4 flex flex-wrap gap-2">
          ...
          <div className="grid gap-4 md:grid-cols-2">
            {platform.engagement.records.map((record) => (
              <article key={record.id} className="rounded-[24px] border border-slate-200 bg-white p-5">
                ...
                <p className="mt-3 text-sm text-slate-500">{record.owner}</p>
                <div className="mt-4 grid grid-cols-2 gap-3 text-sm">
                  <div className="rounded-2xl bg-slate-50 px-4 py-3">{record.participation}% participation</div>
                  <div className="rounded-2xl bg-slate-50 px-4 py-3">{record.sentimentScore}% sentiment</div>
                </div>
            ...
            <tbody>
              {platform.compliance.items.map((item) => (
                <tr key={item.id} className="border-t border-slate-100">
                  <td className="px-4 py-4 font-semibold text-slate-900">{item.policy}</td>
                  <td className="px-4 py-4">{item.owner}</td>
                  <td className="px-4 py-4">{dateLabel(item.dueDate)}</td>
                  <td className="px-4 py-4">
          ...
          {platform.exits.records.map((record) => (
            <article key={record.id} className="rounded-[24px] border border-slate-200 bg-white p-5">
              <div className="flex items-start justify-between gap-4">
                <div>
                  <p className="font-semibold text-slate-900">{record.employeeName}</p>
                  <p className="text-sm text-slate-500">Last working day {dateLabel(record.lastWorkingDay)}</p>
                </div>
                <StatusBadge label={record.stage} />
              </div>
              <div className="mt-4 grid gap-3 sm:grid-cols-3">
                <div className="rounded-2xl bg-slate-50 px-4 py-3 text-sm">Settlement: {record.settlementStatus}</div>
                <div className="rounded-2xl bg-slate-50 px-4 py-3 text-sm">Assets: {record.assetReturn}</div>
                <div className="rounded-2xl bg-slate-50 px-4 py-3 text-sm">Interview: {record.exitInterview ? 'Done' : 'Pending'}</div>
              </div>
            </article>
          ))}
        </div>
            ...
            <table className="min-w-full text-left text-sm">
              ...
              <tbody>
                {platform.budget.records.map((record) => (
                  <tr key={record.department} className="border-t border-slate-100">
                    <td className="px-4 py-4 font-semibold text-slate-900">{record.department}</td>
                    <td className="px-4 py-4">{currency(record.allocated)}</td>
                    <td className="px-4 py-4">{currency(record.spent)}</td>
                    <td className="px-4 py-4">{currency(record.hiringCost)}</td>
                    <td className="px-4 py-4">{currency(record.forecastNextQuarter)}</td>
                    <td className="px-4 py-4">{record.roi}x</td>
                  </tr>
                ))}
              </tbody>
            </table>
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-3xl font-bold text-slate-950">IIT Help Desk Dashboard</h2>
          <p className="text-slate-500">Global overview of all employee tickets and system issues.</p>
        </div>
        <button 
          onClick={loadIssues}
          className="rounded-xl bg-slate-100 p-2 hover:bg-slate-200 transition"
        >
      ...
      <SectionCard title="Global Ticket Registry" subtitle="Manage and assign tickets across the organization.">
        <div className="table-shell overflow-x-auto">
          <table className="min-w-full text-left text-sm">
            ...
            <tbody>
              {issues.map((issue) => (
                <tr key={issue.id || issue._id} className="border-t border-slate-100 hover:bg-slate-50 transition-colors">
                  <td className="px-4 py-4 font-mono text-xs text-slate-400">#{(issue.id || issue._id || "").slice(0, 8)}</td>
                  <td className="px-4 py-4">
                    <p className="font-semibold text-slate-900">{issue.createdBy?.name || 'Unknown'}</p>
                    <p className="text-[10px] text-slate-500 uppercase font-bold">{issue.createdBy?.email || 'No email'}</p>
                  </td>
                  <td className="px-4 py-4 font-medium text-slate-700">{issue.title}</td>
                  <td className="px-4 py-4">
                    <StatusBadge label={issue.category?.toUpperCase()} />
                  </td>
                  <td className="px-4 py-4">
                    <span className={`rounded-full px-2 py-0.5 text-[10px] font-bold uppercase ${
                      (issue.priority || issue.severity) === 'critical' ? 'bg-rose-100 text-rose-700' : 
                      (issue.priority || issue.severity) === 'high' ? 'bg-orange-100 text-orange-700' :
                  ...
                  <td className="px-4 py-4 text-slate-500">{new Date(issue.createdAt || issue.created_at).toLocaleDateString()}</td>
                </tr>
              ))}
      ...
      <SectionCard title="Live workforce movement" subtitle="Real-time geographic distribution and activity status.">
        <div className="table-shell overflow-x-auto">
          <table className="min-w-full text-left text-sm">
            ...
            <tbody>
              {locations.map((loc) => (
                <tr key={loc.userId} className="border-t border-slate-100">
                  <td className="px-4 py-4">
                    <p className="font-semibold text-slate-900">{loc.name}</p>
                    <p className="text-xs text-slate-500">ID: {loc.employeeId}</p>
                  </td>
